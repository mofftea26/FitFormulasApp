...
import { BodyCompForm, bodyCompSchema } from "@/utils/form";

export default function BodyCompCalculator() {
  ...
  const onSubmit = async (values: BodyCompForm) => {
    try {
      const res = await fetch(
        "https://gxrxyjeacovzbmczydgw.supabase.co/functions/v1/lbm-fatmass",
        {
          method: "POST",
    ...
    <ThemedView>
      ...
      <View style={styles.row(spacing)}>
        {(["male", "female"] as const).map((g) => (
          ...
          >
            <ThemedText
              style={styles.optionText(typography, colors, gender === g)}
            >
              {g}
            </ThemedText>
          </TouchableOpacity>
